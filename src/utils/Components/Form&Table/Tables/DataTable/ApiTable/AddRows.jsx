...
import {
  AddRowColumn,
  AddRowData,
} from "@/Data/Form&Table/Table/DataTable/ApiDataTablesData";
import { AddNewRowButton, SearchTableButton } from "@/Constant/constant";
...
const AddRows = () => {
  ...
  const filteredItems = tableData.filter(
    (item) =>
      item.column1 &&
      item.column1.toString().toLowerCase().includes(filterText.toLowerCase())
  );
      ...
      <Card>
        <CardHeader>
          ...
          <span>
            ...
            API method. Simply call the API function with the data for the new
            row (be it an array or object). Multiple rows can be added using the
            <code className="api" title="DataTables API method">
              rows.add()
            </code>
            method (note the plural). Data can likewise be updated with the
            <code className="api" title="DataTables API method">
              row().data()
            </code>
            and
          ...
          <span>
            ...
            method, which is easily done through the chaining that the
            DataTables API employs.
          </span>
        </CardHeader>
        <CardBody>
