...
} from "@/Redux/Reducers/FormLayout/StudentWizardSlice";
      ...
      <div>
        <div className="text-end pt-3">
          {level > 1 && (
            <Button
              color="secondary"
              onClick={() => dispatch(handleStudentBackButton())}
            >
              {Previous}
            </Button>
          )}
          <Button
            color="primary"
            className={`ms-1 ${level === 4 ? "disabled" : ""}`}
            onClick={() => dispatch(handleStudentNextButton())}
          >
            {Next}
          </Button>
        </div>
