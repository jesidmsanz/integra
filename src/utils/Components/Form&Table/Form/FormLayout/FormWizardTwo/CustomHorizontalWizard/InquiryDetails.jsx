...
export const InquiryDetails = ({ getUserData, differentId }) => {
  const { inquiresForm } = useAppSelector((state) => state.formWizardTwo);
  const { notifications, email, contactNumber } = inquiresForm;
  const itemsName = ["Featured Items", "Newsletters", "Notifications", "Blogs"];
          ...
          <div className="choose-option">
            {itemsName.map((data, index) => (
              <FormGroup check key={index} className="radio radio-primary">
                <Input
                  className="me-2"
                  id={differentId ? data : `${data + index}`}
                  type="radio"
                  name="notifications"
                  onChange={getUserData}
                  checked={notifications === data}
                  value={data}
                />
                <Label check htmlFor={differentId ? data : `${data + index}`}>
                  {data}
                </Label>
              </FormGroup>
            ))}
