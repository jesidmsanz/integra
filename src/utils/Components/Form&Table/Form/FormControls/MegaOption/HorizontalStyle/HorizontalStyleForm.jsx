import { HorizontalStyleDataList } from "@/Data/Form&Table/Form/FormData";
                ...
                <Col sm="9" key={id} className={colClass}>
                  <Card className={cardClass}>
                    <div className="d-flex p-20">
                      <FormGroup
                        className={`radio radio-${color} m-0 w-100`}
                        check
                      >
                        <Input
                          id={`radioh-${id}`}
                        ...
                        >
                          <span className={`flex-grow-1 ${mediaBodyClass}`}>
                            ...
                            <span className={spanClass}>
                              {star &&
                                star.map(({ id, starClass }) => (
                                  <i
                                    key={id}
                                    className={`icofont icofont-star ${starClass}`}
                                  ></i>
                                ))}
                              {spanText}
                            </span>
